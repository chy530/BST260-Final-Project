...
salteam %>% ggplot(aes(yearID,p90rat,fill=factor(yearID))) + 
  ...
  labs(title="Ratio of 90th:50th percentile salaries",x="Year",y="90th percentile ÷ median salary")
```
...
warteam %>% ggplot(aes(yearID,medWAR,fill=factor(yearID))) + 
  geom_boxplot(na.rm = TRUE) + theme(legend.position="none") + 
  scale_x_continuous(breaks = seq(1985,2015,5)) + 
  labs(title = "Wins Above Replacement (WAR)",y = "Median WAR (by team)")
...
warteam %>% ggplot() + geom_tile(aes(x=yearID,y=factor(franfct),fill=q75WAR)) + 
  ...
  scale_fill_gradient(low="#00007F", high="red", name="WAR") +
  geom_vline(aes(xintercept=1994),linetype=3,size=1, color="white") +
  labs(title="75th percentile WAR by team",x="Year",y="Team/Franchise")
```
...
But: do better teams win?

```{r,echo=FALSE,message=FALSE}
warteam %>% ggplot() + geom_point(aes(WARtot,wlratio)) +
  geom_smooth(aes(WARtot,wlratio))
```
...
for(i in 1:6){
  tmpwar <- warteam %>% filter(yearID > dates$date1[[i]] & yearID <= dates$date2[[i]])
  wlres <- tmpwar %>% lm(wlratio ~ WARtot,data=.)
  print(summary(wlres))
  label = paste(dates$date1[[i]],dates$date2[[i]],sep="-")
  myplot <- tmpwar %>% ggplot() + geom_point(aes(WARtot,wlratio)) + geom_smooth(aes(WARtot,wlratio)) + labs(title=label,x="Team Total WAR",y="Win/Loss Ratio")
